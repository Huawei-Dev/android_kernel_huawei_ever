					    ...
					    unsigned long flags)
{
	...
	/*
	 * we must zero buffer if heap has buffer_zero,
	 * we do this to avoid user get dirty pages
	 */
	if (heap->ops->buffer_zero) {
		if (buffer->size >= HISI_ION_FLUSH_ALL_CPUS_CACHES) {
			...
			if (flags & ION_FLAG_CACHED)
				dma_sync_sg_for_device(&hisi_ion_dev->dev,
						       table->sgl, table->nents,
						       DMA_BIDIRECTIONAL);
		...
		/*
		 * sec heap buffer no need to deal with cache operation
		 * DMA HEAP do cache operation by itself
		 */
			     ...
			     unsigned int flags, bool grab_handle)
{
	...
	if (heap_id_mask & 0x1 << ION_IRIS_HEAP_ID)
		heap_id_mask |= 0x1 << ION_IRIS_DAEMON_HEAP_ID;
...
int ion_sync_for_cpu(struct ion_client *client, int fd)
{
	...
	if (buffer->size >= HISI_ION_FLUSH_ALL_CPUS_CACHES)
		...
		dma_sync_sg_for_cpu(&hisi_ion_dev->dev, buffer->sg_table->sgl,
